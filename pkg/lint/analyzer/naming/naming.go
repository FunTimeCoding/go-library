...
var suggestions = map[string][]string{
	...
	"req":    {"r", "request"},
	"doc":    {"d", "document"},
	"config": {"c", "configuration"}, // configuration is long but beats a vague abbreviation in non-variable contexts; often replaceable with something more specific like setting
	"cfg":    {"c", "configuration"},
	"llm":    {"m", "model"},
...
func check(pass *analysis.Pass, ident *ast.Ident, obj types.Object) {
	...
	for _, segment := range segments(ident.Name) {
		...
		pass.Reportf(ident.Pos(), "%s", formatMessage(applicable, segment, ident.Name))

		return
	}
}

func formatMessage(applicable []string, segment, name string) string {
	if len(applicable) == 1 {
		return fmt.Sprintf("use %q instead of %q in %s", applicable[0], segment, name)
	}
...
func init() {
	...
	for k, v := range suggestions {
		entries = append(entries, fmt.Sprintf("%s -> %s", k, strings.Join(v, " or ")))
	}
