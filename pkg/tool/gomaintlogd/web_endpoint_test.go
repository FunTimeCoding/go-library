...
func TestWebEndpoint(t *testing.T) {
	...
	assert.HTTPStatus(t, fmt.Sprintf("%s/entries", base), http.StatusOK)
	assert.HTTPStatus(t, fmt.Sprintf("%s/add", base), http.StatusOK)
	// Dashboard shows empty state
	body := getBody(t, fmt.Sprintf("%s/", base))
	...
	body = getBody(t, fmt.Sprintf("%s/entries", base))
	assert.StringContains(t, "worker1", body)
	// Detail fragment
	body = getBody(t, fmt.Sprintf("%s/entry/detail?id=1", base))
	...
	// Edit form fragment
	body = getBody(t, fmt.Sprintf("%s/entry/edit?id=1", base))
	assert.StringContains(t, "restarted web server", body)
	...
	// Delete via POST
	r, e = http.Post(fmt.Sprintf("%s/entry/delete?id=1", base), "", nil)
	assert.FatalOnError(t, e)
	assert.Integer(t, http.StatusOK, r.StatusCode)
